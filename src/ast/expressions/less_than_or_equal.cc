...
bool LessThanOrEqualExpression::Check() {
    ...
    {
        Error::ShowError("Only numerical values can be compared with LESS THEN OR EQUAL (<=) operator!",this->location);
        return false;
    }
...
{
    throw std::logic_error("LessThanOrEqualExpression::TranslateToIR() should not be called!");
}