    ...
    */
    ...
    else if (auto destMem = std::dynamic_pointer_cast<IRMem>(moveExp->destination))
    {
        ...
        if(destBinOp)
        {
            ...
            if(std::dynamic_pointer_cast<IRBinaryOperator>(destBinOp->right) || std::dynamic_pointer_cast<IRMem>(destBinOp->left))
            {
                goto no_pattern_matched;
            }

            if(std::dynamic_pointer_cast<IRMem>(destBinOp->left))
            {
                // TODO: Impelement for object offsetting
            }
